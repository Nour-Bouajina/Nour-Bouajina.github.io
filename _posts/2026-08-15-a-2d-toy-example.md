...
**Demo B**, $\theta_1^2+\theta_2^2$, gives a *continuous* solution manifold: a whole circle of solutions per target. This is the closer analogue to the real project. 42 knobs against 400 output constraints generically leaves a positive-dimensional solution set too (not isolated points), exactly like 2 knobs against 1 output constraint leaves a 1-dimensional circle ($2-1=1$).
...
$S_B(\theta) = \theta_1^2+\theta_2^2$. For target $y^\star = 4$, the exact solution set is $\theta_1^2+\theta_2^2 = 4$, a circle of radius 2. The physics loss $d(\theta) = (S_B(\theta) - y^\star)^2$ plotted over the plane is a ring-shaped valley, not a single minimum, the picture that motivates everything downstream.
...
With only 2 dimensions, $p(\theta\mid y^\star) \propto p(\theta)\, e^{-\beta\, d(S(\theta),y^\star)}$ can be evaluated on a grid and shown as a heatmap, no sampling needed.
...
`ConditionalFlow2D` in the notebook is a direct 2-D shrink of the real project's conditional normalizing flow: affine coupling layers conditioned on an embedding of $y^\star$, a tanh squash into $(-3, 3)^2$ matching the uniform prior's support (same role as the real model's sigmoid squash into $[\theta_{\min},\theta_{\max}]$), both directions implemented with log-determinants tracked through the change-of-variables formula.

**Self-consistency check**: `sample()`'s forward-pass $\log q_\phi$ and `log_prob()`'s independent inverse-pass evaluation agreed to **2.4e-7** on a random batch. The two code paths compute the same density.
...
A single $q_\phi(\theta\mid y)$ was trained on $y^\star\in\{1, 2, 4, 9\}$ (radii 1, 1.414, 2, 3) at once, then tested on those four plus one **unseen** value, $y^\star = 6.25$ (radius 2.5), never shown during training.

| $y^\star$ | true radius | mean generated radius | mean $\lvert S(\theta)-y^\star\rvert$ |
|---|---|---|---|
...
| $p(\theta\mid y^\star)$ and the role of $\beta$ | grid heatmaps, broad at $\beta=0.1$ to razor-thin at $\beta=100$ |
| role of the prior | uniform vs. Gaussian prior visibly reshaping (not invalidating) the ring |
| conditional normalizing flow, $\dim z=\dim\theta$ | `ConditionalFlow2D`, verified self-consistent to 2.4e-7 |
